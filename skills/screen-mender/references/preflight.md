...
| 5 | ❌ | 相依 skill·agent | sibling skill `add-snapshot`/`screen-list`/`shot-audit` ＋ agent `screen-mender-runner`·`screen-mender-integrator`（及其 `agents/references/01..06-*.md` 階段檔）皆在 | 任一缺→硬缺終止（依賴缺失無法起動；integrator 缺會拖到 run 最尾才爆，最貴） |
