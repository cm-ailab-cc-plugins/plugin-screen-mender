...
## Exit
- **canary 過閘**（prompt `canary=true` 且 build+出圖成功）→ status `canary-ok`，**立即早退**（不進偵測/修復/定稿），交 orchestrator 放開其餘 lane。worktree 已暖，後續同畫面正常 runner 走 warm 重用。
...
- 上述（harness-missing 除外，那是停 run）列 backlog、結束本畫面（其餘 TODO 標掉、不納入彙整）。
