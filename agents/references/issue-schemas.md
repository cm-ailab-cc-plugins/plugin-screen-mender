...
### 可見 delta 鐵則：修法只在「截圖真的變了」時才算數

一個修法只有在 after 圖（目標區）相對 before **出現預期的可見變化**時才成立。

- **after 與 before 視覺相同 → 修法無效，還原該變更、不得保留、不得當已修。**
  - 成因不外乎：改錯節點（編輯到的不是實際 render 該畫面的元素）、no-op、或 harness 遮蔽（如 `navigationBarsPadding` 在非 edge-to-edge host 報 0、`Locale.current` 未套、native API 在 sim 顯舊值）。
  - 若確認是「修法對、但 harness 渲染不出」→ 標 `snapshot-unverifiable`（§3.5）、escalate／defer 真機；**仍然還原這個無效變更**，不得把「收緊 UI 卻零可見效益」的改動（`maxLines=1`、改 `padding` 語義…）留在 diff。
- 鐵則理由：本 skill 只改「截圖看得見」的東西。修法的依據是 **before→after 的可見差異**，不是「推敲 code 哪裡可能有問題」就改了放著——後者會把 UI 綁得更死，卻沒修到任何看得見的缺陷（過度修復）。

### 修一個元素不得截斷另一個（搬移截斷 = 假修復）

為了把空間讓給目標元素，而壓縮／截斷**同容器的鄰居**（典型：給鄰居 `Text` 加 `weight(fill=false)` + `maxLines=1` + `ellipsis`，或 reflow 把鄰居擠到截斷）：

- 若該鄰居在 before **本來完整可讀**、after 變成「Resu…」這類新截斷 → **不算修好**，只是把截斷從目標搬到鄰居（同「換位置」假修復）。
- 鐵則：**before 完整可讀的元素，after 必須仍完整可讀**；對「之前沒截斷」的元素引入新的 `…`／裁切／clipping = FAIL。
- 「非操作性／裝飾性」不是可截斷的理由——名稱／標題類（如模板名、卡片標題）屬內容，截斷即資訊損失。
- 空間真的不夠塞下全部 → 走更高階解（整列換行、堆疊改版、縮字串），或 `deferred:needs-design`；不得犧牲鄰居可讀性硬塞。
- 此規則對 audit／fix／verify 三階段都成立：audit 不得把「截斷鄰居」寫進修法 hint／AC；fix 不得執行；verify 不得通過（即使該鄰居落在 target 區、即使 AC 寫了「可接受壓縮」——那種 AC 本身有問題，判 `AUDIT_PROBLEM`）。
...
- **借鄰居的空間不得截斷鄰居**：靠壓縮同列另一個元素（weight fill=false / maxLines=1 / ellipsis）來騰位給目標時，該鄰居若從 before 完整變成 after 截斷 = 搬移截斷、不算修好（詳見上〈修一個元素不得截斷另一個〉）。空間不夠走整列換行／縮字串／`deferred:needs-design`。
