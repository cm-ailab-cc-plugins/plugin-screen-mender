...
runner 端只保留 self-abort：fix 達 `iterate_max`／build 同錯連 3 次／內部迴圈超 `internal_loop_max_rounds`／審查驗證階段 AUDIT_PROBLEM → 停止，用 return 的 `escalation` 回報；orchestrator 收到即上報使用者。
...
`verify_verdict` PASS（scope/redesign 守住 + 被修的那幾條 AC + 目標區正確 + 視覺等價 + 無 regression，審查與驗證一格涵蓋）→ `<mr_tool> mr update <id> --ready`。

- verify PASS ≠ 畫面乾淨；畫面狀態由 mr 階段綜合 verify 的 `residual_visible` 另算（§5.3）。
- self-review/verify 紀律：審查與驗證階段雖與 fix 是同一 runner，仍須換「審查者／驗收者」視角獨立審 scope/redesign + 比 AC + 視覺等價 + 殘留盤點，**不得因自己是修的人就放水或略過會出錯處**（見 `agents/references/04-verify.md`）。這是合併 agent 後最需守住的一關。
...
`fix ↔ 審查驗證`（review + verify 已併為一格）最多 `internal_loop_max_rounds`（內建預設 3）輪。超過 → runner return `escalation`（含 round 紀錄）+ orchestrator 上報使用者（不寫檔），不無限迴圈。
...
每畫面 runner 回一筆 `{capture_s, audit_s, fix_s, verify_s, build_runs, dev_rounds}`（`verify_s` 涵蓋審查＋驗證）→ 供 Phase 3 final summary 呈現（compact 一行；`trace=true` 出完整逐階段 breakdown）。觀測資料只進 final summary（對話呈現），不寫 `.audit`。
