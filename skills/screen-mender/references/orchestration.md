...
orchestrator(main session) 只做：配 lane、派 sub-agent、整合結果、開 MR（不維護 ledger、不輪詢 merge）。

任何 codebase 修改 / build / test / 截圖 / rebase / push 一律委派背景 sub-agent。

- 原因：main session 不自己跑會 block > 5 min 的工作。

## 2. 背景 agent 生命週期（無 watchdog）

orchestrator 不做 heartbeat 輪詢 / ScheduleWakeup watchdog。

- happy path：背景 sub-agent 完成時 harness 自動通知 orchestrator（完成通知驅動），不需輪詢。
- backstop：極端「卡死不回報」靠本 skill 互動觸發、使用者在線，無進度即打斷。

agent 端只保留 self-abort：同一錯誤連 3 次 → 停止重試，用 return 值回報；orchestrator 收到 stuck return 即上報使用者。

- return 值四項：stuck、卡點、試過什麼、建議。
...
原子認領 = `mkdir` + 立刻 spawn，同一個 action：

- `mkdir` atomic 當搶占宣告，在內建 claim 目錄（`<repo>/.screen-mender/claims`）；鎖目錄則用內建預設 `<repo>/.screen-mender/locks`。
- 鐵則：認領與「spawn 該畫面第一個 agent」必須在同一輪 tool-call 內完成——不可只 `mkdir`/敘述「已認領、待會 launch」卻把 spawn 的 tool call 漏掉（曾因此讓一條 lane 閒置數十分鐘才被發現補發）。

```
for unified_id in manifest（優先挑與本 lane worktree 當前 branch 同 module 者，減少 branch churn）:
    mkdir <claim_dir>/<unified_id>/   成功 → 這個歸我，**同一輪立刻 spawn 它的第一個 agent**（Phase 2→4）
                                      失敗（已被別 lane 搶）→ 試下一個
全 manifest 都認領不到 → 本 lane 收工
```

claim↔live-agent 對賬（事件驅動，非輪詢）：orchestrator 每次被完成通知喚醒、或準備讓某 lane 收工前，對照 `claim_dir` 既有 claim 與目前 live agent；有 claim 卻無對應 live agent = 漏派，立即補 spawn。這不是 watchdog（不定時主動喚醒、不寫 heartbeat），是事件驅動下的一致性對賬。

無跨-lane 裝置鎖：每 lane 各有一台 emulator，lane 內 capture 與 verify 本就序列（同一畫面先 capture 後 verify），故不需單裝置 singleton / capture-verify 互斥鎖。
...
- developer 只 local commit、**不 push**（orchestrator 傳 `dry_run` 給 developer）。
- orchestrator **不 rebase、不 push、不開 MR**；改從 lane worktree 取產物落 `<run_dir>/<unified_id>/`：
  - `change.patch` = `git -C <worktree> format-patch <base_branch>..HEAD --stdout`。
  - `before.png` / `after.png`（複製 developer 的 before/after 截圖）。
...
- reviewer 照跑，`diff_cmd` = `git -C <worktree> diff <base_branch>`；verifier 照判 after-shot；內部 loop 照常。
...
developer push 後，orchestrator rebase 到 `base_branch`（已 push 用 `--force-with-lease`），開一個 MR（一畫面一個）。

- rebase 一律由 orchestrator 做；developer（含被退回重修那輪）只 commit + push，不自己 rebase——避免雙方對同一 branch 互踩 force-push。
...
reviewer PASS + verifier PASS（被修的那幾條 AC + 目標區正確 + 視覺等價 + 無 regression）→ `<mr_tool> mr update <id> --ready`。

- verifier PASS ≠ 畫面乾淨；畫面狀態由 orchestrator 綜合 verifier 的 `residual_visible` 另算（§5.3）。
- 派 verifier 紀律（不可劇透 + 豁免）：verifier prompt 只給 AC + before/after + 哪裡是目標區；不得預先宣告「哪些變化可接受 / 不要 flag」——那會關掉它對會出錯處的審視。
...
developer ↔ reviewer / developer ↔ verifier 各自最多 `internal_loop_max_rounds`（內建預設 3）輪。超過 → return 回報 round 紀錄 + 上報使用者（不寫檔），不無限迴圈。

上限 `internal_loop_max_rounds`（內建預設 3）適用對象（各自獨立計）：
...
orchestrator 對每畫面記錄各階段耗時（不靠 agent 自報，避免漏算 spawn 延遲）：

- spawn 每個 sub-agent（capture / audit / developer / reviewer / verifier）前取一次 `date +%s`，收到該 agent 完成通知時再取一次，差值 = 該階段 wall-clock。時間戳只在既有 spawn／完成事件點順手取，不新增喚醒、不輪詢。
- build 次數：加總 developer return 的 `build_runs` + verifier return 的 `build_runs`（+ capture 階段本身一次 build）。
- 迴圈輪數：developer↔reviewer、developer↔verifier 實際輪數（orchestrator 本就掌握）。

每畫面彙整一筆 `{capture_s, audit_s, fix_s, review_s, verify_s, build_runs, dev_rounds}` → 供 Phase 5 final summary 呈現（compact 一行；`trace=true` 出完整逐階段 breakdown）。觀測資料只進 final summary（對話呈現），不寫 `.audit`。
