...
| 整體 skill 跑 | 25 分鐘 | 用 return 值 / stdout 上報「skill 在 framework `<FrameworkCell>` 上撞壁」+ 試過什麼 + 建議；不寫任何紀錄檔 |
...
完整對照表（用途 / 範例 / runtime 代入規則）見 [`references/placeholders.md`](references/placeholders.md)——填 host / test 範本時讀。
...
新 App 第一次用本 skill 的一次性 setup 見 [`references/setup.md`](references/setup.md)（本 skill plugin 內建，不需複製、不需改 skill 檔；下列只確認專案前置）：
