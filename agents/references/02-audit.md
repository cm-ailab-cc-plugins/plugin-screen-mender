...
### 1. 偵測
- 用 Skill `shot-audit` 偵測本畫面所有截圖看得見的視覺缺陷（含描述 + 調整建議）。
- 帶 `layout_stress_locale = capture_locale` + 兄弟畫面參考。
...
- `deferred`：真缺陷、本 run 不修。
  ...
  - → after 圖仍可見、列殘留可見、畫面降 `partially-fixed`，不得偽裝成已修或與 needs-design 混用。
...
- 結構性、需設計決策但無設計來源證據 → 記 summary，不靜默吞：
  ...
  - `deferred:needs-design`（是 bug、修法需設計）。
